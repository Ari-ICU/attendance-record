...
const { authMiddleware, adminOnly, managerOrAdminOnly } = require('../middlewares/auth.middleware');

router.use(authMiddleware);

router.post('/', overtimeController.createOvertime);
router.get('/', overtimeController.getAllOvertime);
router.patch('/:id/status', managerOrAdminOnly, overtimeController.updateOvertimeStatus);
