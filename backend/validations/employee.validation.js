...
const employeeCreateSchema = Joi.object({
    ...
    phone: Joi.string().trim().optional().allow('', null).default('+855 12 000 000'),
    position: Joi.string().trim().optional().allow('', null).default('Staff Member'),
    department: Joi.string().trim().optional().allow('', null),
    type: Joi.string().valid('employee', 'student').default('employee'),
    dateOfJoining: Joi.date().optional().default(Date.now),
